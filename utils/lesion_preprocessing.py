...
def training(train_data_dir, save_name=""):
    ...
    with dpp.Pipeline(storage_name="", initialization_generator=init) as pipe:
        ...
        #node = seg.random_rotation(node, probability=0.5, upper_bound=180)
        #node = seg.crop_to_label(node, 1, max_reduction=10, square_crop=True, default_label=0)

        # Random transformations
        node = seg.random_resize(node, [256, 256], probability=0.6, lower_bound=0.8, upper_bound=1.1, default_pixel=0., default_label=0)
