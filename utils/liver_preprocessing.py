...
def training(train_data_dir, save_name=""):
    ...
    with dpp.Pipeline(storage_name=save_name, initialization_generator=init) as pipe:
        ...
        #node = seg.random_translation(node, probability=0.1, border_usage=0.5, default_border=0.25, label_of_interest=1, default_pixel=-100., default_label=0)
...
def _validation(validation_data_dir, save_name="", label_required=1):
    ...
    with dpp.Pipeline(storage_name=save_name, initialization_generator=init) as pipe:
        node = dpp.reader.file_paths(validation_data_dir, input_identifier=input_identifier, ground_truth_identifier=ground_truth_identifier, random=False, iterations=0, split=split, split_selector=1, seed=seed)
